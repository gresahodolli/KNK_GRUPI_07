...
public class Test extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Scene scene=new Scene(login,1000,650);
        ...
        scene.setOnKeyPressed(event -> {
            if(event.getCode()== KeyCode.ENTER){
                if(LogIn.loggingIn(login.getTfUsername().getText(),login.getPfPassword().getText())==1){
                    ...
                    Scene adminScene=new Scene(new AdminView(adminStage),600,600);
        ...
        login.getBtnLogin().setOnAction(event -> {
            if(LogIn.loggingIn(login.getTfUsername().getText(),login.getPfPassword().getText())==1){
                Stage adminStage=new Stage();
                Scene adminScene=new Scene(new AdminView(adminStage),1000,650);
                adminStage.setScene(adminScene);
                adminStage.setTitle("Admin");
                adminStage.show();
                primaryStage.hide();
                ...
                managerStage.show();
                primaryStage.hide();
            }else if(LogIn.loggingIn(login.getTfUsername().getText(),login.getPfPassword().getText())==3) {
                    int userId=LogIn.getUserIdByUsername(login.getTfUsername().getText());
                    Stage regularStage = new Stage();
                    Scene regularScene = new Scene(new RegularUI(regularStage));
